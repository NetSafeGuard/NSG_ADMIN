      ...
      <C.ActivityContainer>
        ...
        {activitys.map((activity, index) => (
          <>
            ...
            {activity.date.getDate() === activitys[index - 1]?.date.getDate() ? (
              <C.ActivityCard>
                <C.ActivityTitle>{activity.title}</C.ActivityTitle>
                <C.ActivityDescription>{activity.description}</C.ActivityDescription>
              </C.ActivityCard>
            ) : (
              <>
                <C.ActivityDate>
                  {
                    activity.date.getDate()} {activity.date.toLocaleString('default', { month: 'short' })
                  }.
                </C.ActivityDate>
                <C.ActivityCard>
                  <C.ActivityTitle>{activity.title}</C.ActivityTitle>
